...
This deployment is used when contracts have inter-dependency, then you can deploy multiple contracts one after another:
...
```typescript
    ...
    #[contract]
    mod SimpleCounterFactory {
        use starknet::ContractAddress;
        use starknet::syscalls::deploy_syscall;

        struct Storage {
            deployed_counters: LegacyMap<u128, ContractAddress>,
            next_id: u128,
        }
        ...
        #[external(v0)]
        fn deploy_new_counter(ref self: ContractState, initial_value: u128) -> ContractAddress {
            let class_hash = <YourSimpleCounterClassHash>; // Replace with actual class hash
            let salt = self.next_id.read();
            let calldata = array![initial_value]; // Constructor arguments for SimpleCounter

            let (contract_address, _) = deploy_syscall(class_hash, salt, calldata.span(), false).unwrap();

            self.deployed_counters.write(salt, contract_address);
            self.next_id.write(salt + 1);

            contract_address
        }
    }
```
More examples can be seen in[Starknet documentation](https://docs.starknet.io/archive/starknet-by-example/factory-pattern/)
...
```
More examples of upgradeable contracts can be found in the [Cairo documentation](https://www.starknet.io/cairo-book/ch103-03-upgradeability.html)
and [Medium documentation](https://medium.com/@EmpiricNetwork/starknet-guide-writing-upgradable-contracts-using-a-proxy-af3f107f238b)
## Best Practice
